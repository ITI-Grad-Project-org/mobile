...
import { METRICS, type MetricMeta } from "./metrics";

/**
 * Derived readings over a measurement history — the shared data layer for a
 * domain both UIs read: the client sees their own on Progress, the coach sees a
 * client's on the check-in detail screen. Screens stay per-UI; only this
 * derivation is shared.
 *
 * The input is deliberately `unknown`: the list endpoint returns either a bare
 * array or a `{ docs, meta }` envelope, so every reader goes through
 * `unwrapList` rather than assuming one.
 */
