import { enduranceExercises, exercises as strengthExercises, yogaExercises, type Exercise } from "@/lib/data";
...
const PLAN_META: Record<string, { title: string; subtitle: string; tone: "mint" | "lilac" | "sky" | "peach"; mins: number }> = {
  strength: { title: "Quad-focused strength", subtitle: "DAY 4 · LOWER BODY", tone: "mint", mins: 55 },
  yoga: { title: "Vinyasa flow", subtitle: "DAY 4 · HIPS & SPINE", tone: "lilac", mins: 45 },
  endurance: { title: "Tempo run + core", subtitle: "DAY 4 · THRESHOLD", tone: "sky", mins: 50 },
  weightloss: { title: "Full-body conditioning", subtitle: "DAY 4 · METABOLIC", tone: "peach", mins: 40 },
};
...
export function ClientTodayScreen() {
  ...
  const [done, setDone] = useState<Record<string, boolean>>({ [exercises[0].id]: true });
    ...
    >
      ...
      <View className="flex-row items-center justify-between px-1">
        ...
        <View className="relative">
          ...
          />
          <View className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full bg-success border-2 border-background" />
        </View>
      </View>
      ...
      <View>
        ...
        <View className="flex-row gap-3">
          <Card tone="peach" interactive className="flex-1 p-5">
            ...
            <Text className="mt-3 text-3xl font-black text-peach-ink">1,840</Text>
            <Text className="text-[12px] text-peach-ink opacity-80">of 2,400 kcal</Text>
          ...
          <Card tone="sky" interactive className="flex-1 p-5">
            <View className="flex-row items-center justify-between">
            ...
            <Text className="text-[12px] text-sky-ink opacity-80">of 3.0L goal</Text>
            <View className="mt-3 flex-row gap-1">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <View
                  key={i}
                  className={cn("h-6 flex-1 rounded-md", i <= 4 ? "bg-sky-ink" : "bg-sky-ink/15")}
                />
              ))}
            </View>
          </Card>
      ...
      <Card tone="lilac" interactive>
        <View className="flex-row items-center justify-between">
          <View className="flex-1 pr-4">
            ...
            <Text className="mt-1 text-[18px] font-bold text-lilac-ink">Sunday · 6 PM</Text>
          ...
          <Pressable className="rounded-full bg-lilac-ink px-4 py-2 active:opacity-90">
            <Text className="text-[12px] font-semibold text-white">Prepare</Text>
          </Pressable>
        </View>
      </Card>
